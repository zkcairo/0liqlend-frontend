...
function MyContractExecutionModal({ isOpen, onClose, account, tokenUsed, category, alloffers }: Props) {
  ...
  const { data: mylendingid_data, isLoading: mylendingid_loading } = useContractRead({
    ...
    abi: MyAbi,
    functionName: "all_lending_offers_len",
    args: [],
    watch: true,
  });
  ...
  const { data: myborrowingid_data, isLoading: myborrowingid_loading } = useContractRead({
    address: contractAddress,
    abi: MyAbi,
    functionName: "all_borrowing_offers_len",
    args: [],
    watch: true,
  });
      ...
      let call = [
        {
          ...
          calldata: [contractAddress, Number(inputAmount)*(10**Number(choosenDecimals)), 0].map((value) => String(value)),
          entrypoint: "approve"
        }
      ];
    ...
    >
      <div className="absolute right-5 top-4">
        ...
        <div className="grid grid-cols-2 gap-x-5">
          ...
          <div className="flex flex-col justify-center">
          <h2>{activeTab === "Just lend" ? maxYouCanLend : maxYouCanBorrow}$</h2>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-x-5">
          <div className="flex flex-col justify-center">
